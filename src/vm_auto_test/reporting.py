...
def _write_sample_artifacts(report_dir: Path, before: str, after: str, sample_stdout: str, sample_stderr: str, logs: tuple[Any, ...]) -> None:
    (report_dir / "before.txt").write_text(before, encoding="utf-8")
    (report_dir / "after.txt").write_text(after, encoding="utf-8")
    (report_dir / "sample_stdout.txt").write_text(sample_stdout, encoding="utf-8")
    (report_dir / "sample_stderr.txt").write_text(sample_stderr, encoding="utf-8")
    if logs:
        logs_dir = report_dir / "av_logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        for log in logs:
            safe_id = "".join(char if char.isalnum() or char in "-_" else "-" for char in log.collector_id)
            (logs_dir / f"{safe_id}_stdout.txt").write_text(log.stdout, encoding="utf-8")
            (logs_dir / f"{safe_id}_stderr.txt").write_text(log.stderr, encoding="utf-8")
